...
14. [Машиночитаемые файлы](#машиночитаемые-файлы)
...
Каждый запуск команды получает отдельный QuickJS engine.
...
Timeout закрывает engine. Незавершённые операции после закрытия не должны считаться выполненными.
...
Строка в примере является base64-представлением JSON `{"generated":true,"value":42}`. Для динамических бинарных данных плагин должен использовать собственный JavaScript-кодировщик base64 или получить уже закодированные данные из API.
